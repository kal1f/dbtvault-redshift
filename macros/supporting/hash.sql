...
{%- if columns is string -%}
    ...
    {%- else -%}
        {%- set escaped_column_str = dbtvault.escape_column_names(column_str) -%}
    {%- endif -%}
    {# Redshfit implementation: Cast to VARCHAR doesnt not require hash_size. Also added outer UPPER()  #}
    {{- "CAST((UPPER({}({}))) AS VARCHAR) AS {}".format(hash_alg, standardise | replace('[EXPRESSION]', escaped_column_str), dbtvault.escape_column_names(alias)) | indent(4) -}}
...
{%- else -%}
    ...
    {%- if is_hashdiff -%}
        {# Redshift implementation: I added outer UPPER() to uppercase the md5 result which is lower case in Redshift  #}
        {{- "CAST(UPPER({}(".format(hash_alg) | indent(4) -}}
    {%- else -%}
        {{- "CAST(UPPER({}(NULLIF((".format(hash_alg) | indent(4) -}}
    {%- endif -%}
    ...
    {%- for column in columns -%}
        ...
        {%- if loop.last -%}
            ...
            {% if is_hashdiff %}
                {# Redshift implementation: Cast to VARCHAR doesnt not require hash_size  #}
                {{- "\n)) AS VARCHAR) AS {}".format(dbtvault.escape_column_names(alias)) -}}
            {%- else -%}
                {# Redshfit implementation: Cast to VARCHAR doesnt not require hash_size  #}
                {{- "\n), '{}'))) AS VARCHAR) AS {}".format(all_null | join(""), dbtvault.escape_column_names(alias)) -}}
            {%- endif -%}
        {%- else -%}
